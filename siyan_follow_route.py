...
#         hud.toggle_info() # 关闭抬头显示
#         world = World(client.get_world(), hud, args)
#         world.camera_manager.toggle_camera()  # 第一人称视角
#
#         # list all vehicles
...
#
#         start_point = world.map.get_waypoint(world.player.get_location())
#         # start_point = spawn_points[0]
#
#         destination = start_point.next(50.0)[0]
#         end_point = start_point.next(50.0)[0].transform.location
...
#                     head_width=2.0, head_length=2.0, fc='red', ec='red')
#
...
def game_loop(args):
    ...
    try:
        ...
        hud.toggle_info()  # 关闭抬头显示
        world = World(client.get_world(), hud, args)
        world.camera_manager.toggle_camera()  # 切换到第一人称视角
        ...
        start_point = world.map.get_waypoint(world.player.get_location())
        destination = start_point.next(50.0)[0]
        end_point = destination.transform.location
        agent.set_destination(end_point, start_point)
        ...
        while True:
            ...
            if agent.done():
                if args.loop:
                    agent.set_destination(random.choice(world.map.get_spawn_points()).location)
                    world.hud.notification("Target reached", seconds=4.0)
                    print("The target has been reached, searching for another target")
                else:
                    print("The target has been reached, stopping the simulation")
            ...
            # 检测当前转向情况
            current_wp = world.map.get_waypoint(world.player.get_location())
            next_wp = current_wp.next(2.0)[0] if current_wp.next(2.0) else None
            ...
            if next_wp:
                ...
                if last_turn_direction is not None:
                    delta_angle = angle - last_turn_direction
                    delta_angle = (delta_angle + np.pi) % (2 * np.pi) - np.pi  # 将角度差值归一化到 [-pi, pi]
                ...
                # 更新最后的转向方向
                last_turn_direction = angle

                # 获取当前摄像头对象并打印图像信息
                # camera_rgb = world.camera_manager.sensor
                #
                ...
                #     camera_rgb.listen(lambda image: process_image(image))
                # # self.sensor.listen(lambda image: CameraManager._parse_image(weak_self, image))
